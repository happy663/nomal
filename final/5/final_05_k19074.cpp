...
{   
    ...
    //ここから問題４（テンプレート画像の読み込み等は上で行っている配列の追加等も行っている）
...
/* 
    ...
    //避難場所を緑で描く
    cv::rectangle(dst_img,min_loc,cv::Point(min_loc.x+template_img.cols,min_loc.y+template_img.rows),CV_RGB(0,255,0),3);

    //避難場所に最も近い端点と遠い端点を結ぶ
    cv::line(dst_img,cv::Point(twodimensions[distance_max_recored].xx,twodimensions[distance_max_recored].yy),
    cv::Point(twodimensions[distance_min_record].xx,twodimensions[distance_min_record].yy),CV_RGB(255,0,0),1,8,0);


    //端点の距離を求める
    double r=hypot(twodimensions[distance_max_recored].xx-twodimensions[distance_min_record].xx,twodimensions[distance_max_recored].yy-twodimensions[distance_min_record].yy);
